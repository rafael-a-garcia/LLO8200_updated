...
sfo <- sf_cluster %>% 
  select(-sf.runid) #drops sf.runid variable from dataset (you can see this in the #vars in sfo vs. sf_cluster)
```
...
Hmm, this gave us WAY different answers. Here's why -- the data aren't strongly separated by various groups, so it depends where the starting points were. If we had "clear" clusters/differentiation, our results would be more stable between our c1 and c2 examples. 
...
With two groups, the cluster locations are pretty stable. The Rand Indices are a measure of similarity of points within the same cluster (like a withinness measure). The higher the rand index, the more similar. At 3 they spread out. With 4 and five there appear to be some relatively steady points, but still it's pretty spread out. 
...
Notice how the sample sizes in each group are stable if you re-run this code, although the group numbers (which are arbitrary) are different after each run. Unlike above, this assigns/classifies people and those classifications are set after the kmeanspp function is run (people don't switch groups if you run it again).
...
```{r}
#Age
prop.table(table(sfo$cluster, sfo$sf.age2544), margin = 1) #ages 25-44

prop.table(table(sfo$cluster, sfo$sf.age4564), margin = 1) #ages 45-64
```
...
```{r}
#Income
prop.table(table(sfo$cluster, sfo$sf.inc150p), margin = 1) %>% 
  kable()
```
...
```{r}
#Checked bags?
prop.table(table(sfo$cluster, sfo$sf.bags), margin = 1) %>% 
  kable()
```
...
```{r}
sum2 <- gather(sum1, -cluster, key = variable, value = value) #this takes our data from wide to long
sum2

sum_total <- sfo %>% 
  summarize_all(mean)

sum_total <- gather(sum_total, key = variable, value = overall_mean) #overall sample means for each variable
sum2 <- left_join(sum2, sum_total, by = "variable") #allows us to compare mean in that cluster to sample mean
```
...
```{r}
sfo$cluster_name <- NA #create new empty variable - note - your clusters may be in a different order; name your clusters accordingly! When I ran this, my cluster 3 had my business travelers, for instance. 
sfo$cluster_name[sfo$cluster == 3] <- "Business Traveler"
sfo$cluster_name[sfo$cluster == 4] <- "Vacationers"
sfo$cluster_name[sfo$cluster == 2] <- "Golden Years"
sfo$cluster_name[sfo$cluster == 1] <- "Aunt Sally Visiting the Kids"

#look at your sfo dataset - make sure the names were assigned properly; should also run a crosstab
table(sfo$cluster, sfo$cluster_name)
```
...
hypo_data <- sf %>% 
  data_grid(.model = mod1, cluster=1:4)
