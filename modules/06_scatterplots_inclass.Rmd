...
```{r}
g1<-g1 + geom_smooth(method="lm") #adds smooth linear regression line (lm = linear model)
g1<-g1 + geom_smooth(method = "loess", color="red") #adds LOESS line
...
```{r}
hypo_data<-data_grid(els, byses1=seq_range(byses1, n=100), bypared) %>% 
  add_predictions(mod_2)
```
