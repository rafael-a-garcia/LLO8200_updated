...
req_text<-"Select * from flights" #this tells the DB that you want to pull everything from the airlines table

#Send query through connection
req<-dbSendQuery(con,req_text)

#Generate dataframe from results - fetch it!
...
delay_summary<-req1_df%>%     #take your new dataset
  group_by(name)%>%         #we want to create a mean by the carrier name
  summarize(avg_delay=mean(dep_delay,na.rm=TRUE))%>%  #create the conditional mean
  arrange(-avg_delay)     #sort in descending order
...
dist_flights<-req1_df%>%
  group_by(name)%>%
  tally()%>%        #gives you the total number in each group_by category
  arrange(-n)
...
```{r}
#Plot average delay by wind speed
g2<-ggplot(data=weather_summary_2,aes(x=wind_speed,y=avg_delay))
g2<-g2+geom_point()
g2<-g2+geom_smooth()
g2
```
