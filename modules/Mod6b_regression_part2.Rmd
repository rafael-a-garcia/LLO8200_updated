...
NOTE: This file begins with the Module 6.4 video on the LMS.
...
The next variable I want to include is `metro`, which is a binary variable set to "Yes" if the area is a metropolitan area and "No" if it is not a metropolitan area. The code below shows the number and proportion of metropolitan and non-metropolitan areas in the dataset. 
```{r}
ad%>%
  filter(!is.na(metro))%>%
  group_by(metro)%>%
...
This shows that about 58% of areas are not metro areas, while 42% are metro areas. Question: WHY bother filtering out the NAs on my metro area? What would happen to my proportions had I not filtered them out? 
...
income_rec<-recipe(income_formula, data=ad_train)%>%
  step_dummy(metro) #this is ONLY for categorical variables!!
```
...
lm_results%>%
  extract_fit_parsnip()%>% #old function was pull_workflow_fit()
  glance()
```
